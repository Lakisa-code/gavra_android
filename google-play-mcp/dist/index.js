        ...
        switch (name) {
            ...
            case "google_get_track_status": {
                ...
                const result = {
                    ...
                    allReleases: trackData.releases?.map((release) => ({
                        name: release.name,
                        versionCodes: release.versionCodes,
                        status: release.status,
                        statusDescription: getStatusDescription(release.status),
            ...
            case "google_list_releases": {
                ...
                const allReleases = tracksResponse.data.tracks?.flatMap((track) => track.releases?.map((release) => ({
                    track: track.track,
            ...
            case "google_get_review_status": {
                ...
                const pendingReleases = tracksResponse.data.tracks?.flatMap((track) => track.releases?.filter((release) => release.status === "inProgress" || release.status === "draft").map((release) => ({
                    track: track.track,
                    trackDescription: getTrackDescription(track.track),
                    name: release.name,
                    versionCodes: release.versionCodes,
                ...
                const liveReleases = tracksResponse.data.tracks?.flatMap((track) => track.releases?.filter((release) => release.status === "completed").map((release) => ({
                    track: track.track,
                    trackDescription: getTrackDescription(track.track),
                    name: release.name,
                    versionCodes: release.versionCodes,
            ...
            case "google_remove_testers": {
                ...
                try {
                    ...
                    const newGroups = currentGroups.filter((g) => g !== null && !emailsToRemove.has(g.toLowerCase()));
                    // Update testers
                    await androidPublisher.edits.testers.update({
                        packageName: PACKAGE_NAME,
                        editId,
            ...
            case "google_set_testers": {
                const { track, emails = [], googleGroups = [] } = args;
                const editResponse = await androidPublisher.edits.insert({
                    packageName: PACKAGE_NAME,
                });
                const editId = editResponse.data.id;
            ...
            case "google_list_reviews": {
                const { maxResults = 10, translationLanguage } = args;
                const params = {
                    packageName: PACKAGE_NAME,
                ...
                const reviews = (response.data.reviews || []).map((review) => {
                    ...
                    return {
                        ...
                        developerComment: review.comments?.find((c) => c.developerComment)?.developerComment?.text,
                    };
                });
                return {
                    content: [
            ...
            case "google_get_review": {
                const { reviewId, translationLanguage } = args;
                const params = {
                    packageName: PACKAGE_NAME,
                    reviewId,
                };
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                developerComment: review.comments?.find((c) => c.developerComment)?.developerComment,
                            }, null, 2),
                        },
                    ],
                };
            ...
            case "google_update_app_details": {
                const { contactEmail, contactPhone, contactWebsite, defaultLanguage } = args;
                const editResponse = await androidPublisher.edits.insert({
                    packageName: PACKAGE_NAME,
                });
                const editId = editResponse.data.id;
                try {
                    ...
                    if (contactPhone)
                        requestBody.contactPhone = contactPhone;
                    if (contactWebsite)
                        requestBody.contactWebsite = contactWebsite;
                    if (defaultLanguage)
                        requestBody.defaultLanguage = defaultLanguage;
            ...
            case "google_update_store_listing": {
                const { language, title, shortDescription, fullDescription } = args;
                const editResponse = await androidPublisher.edits.insert({
                    packageName: PACKAGE_NAME,
                });
                const editId = editResponse.data.id;
                try {
                    ...
                    if (shortDescription)
                        requestBody.shortDescription = shortDescription;
                    if (fullDescription)
                        requestBody.fullDescription = fullDescription;
            ...
            case "google_list_apks": {
                ...
                try {
                    ...
                    const apks = (apksResponse.data.apks || []).map((apk) => ({
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                generatedApks: generatedApks.map((apk) => ({
                        ...
                        content: [
                            {
                                ...
                                text: JSON.stringify({
                                    ...
                                    countries: countries.slice(0, 50).map((c) => c.countryCode),
            ...
            case "google_promote_release": {
                const { fromTrack, toTrack, userFraction } = args;
                const editResponse = await androidPublisher.edits.insert({
                    packageName: PACKAGE_NAME,
                });
                const editId = editResponse.data.id;
                try {
                    ...
                    const sourceRelease = sourceTrackResponse.data.releases?.find((r) => r.status === 'completed' || r.status === 'inProgress');
            ...
            case "google_halt_release": {
                ...
                try {
                    ...
                    const activeRelease = trackResponse.data.releases?.find((r) => r.status === 'inProgress');
                    if (!activeRelease) {
            ...
            case "google_resume_release": {
                ...
                try {
                    ...
                    const haltedRelease = trackResponse.data.releases?.find((r) => r.status === 'halted');
                    if (!haltedRelease) {
            ...
            case "google_complete_rollout": {
                ...
                try {
                    ...
                    const activeRelease = trackResponse.data.releases?.find((r) => r.status === 'inProgress' || r.status === 'halted');
                    if (!activeRelease) {
            ...
            case "google_list_inapp_products": {
                ...
                const products = (response.data.inappproduct || []).map((product) => {
            ...
            case "google_list_voided_purchases": {
                const { maxResults = 100, startTime, endTime } = args;
                const params = {
                    packageName: PACKAGE_NAME,
                    maxResults,
                };
                if (startTime)
                    params.startTime = new Date(startTime).getTime().toString();
                if (endTime)
                    params.endTime = new Date(endTime).getTime().toString();
                const response = await androidPublisher.purchases.voidedpurchases.list(params);
                const voidedPurchases = (response.data.voidedPurchases || []).map((purchase) => ({
            ...
            case "google_list_images": {
                ...
                try {
                    ...
                    const images = (imagesResponse.data.images || []).map((image) => ({
                        id: image.id,
                        url: image.url,
                        sha1: image.sha1,
                        sha256: image.sha256,
            ...
            case "google_delete_image": {
                const { language, imageType, imageId } = args;
                const editResponse = await androidPublisher.edits.insert({
                    packageName: PACKAGE_NAME,
                });
                const editId = editResponse.data.id;
            ...
            case "google_upload_image": {
                const { language, imageType, imagePath } = args;
            ...
            case "google_verify_product_purchase": {
                const { productId, purchaseToken } = args;
            ...
            case "google_verify_subscription_purchase": {
                const { subscriptionId, purchaseToken } = args;
            ...
            case "google_defer_subscription": {
                const { subscriptionId, purchaseToken, expectedExpiryTime, desiredExpiryTime } = args;
                        ...
                        content: [
                            {
                                ...
                                text: JSON.stringify({
                                    ...
                                    apks: apksResponse.data.apks?.map((a) => ({
                                        versionCode: a.versionCode,
                                        sha256: a.binary?.sha256,
                                    })) || [],
                                    bundles: bundlesResponse.data.bundles?.map((b) => ({
            ...
            case "google_list_user_comments": {
                const { maxResults = 20, startIndex = 0 } = args;
            ...
            case "google_search_error_reports": {
                const { filter, pageSize = 20 } = args;
                try {
                    const reporting = await getPlayDeveloperReporting();
                    // Use type assertion to handle API typing issues
                    const searchFn = reporting.vitals.errors.reports.search;
