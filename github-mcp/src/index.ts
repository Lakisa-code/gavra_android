        ...
        inputSchema: {
            ...
            properties: {
                secrets: {
                    type: "object",
                    description:
                        "Object with secret names as keys and values as secret values",
                },
            },
            required: ["secrets"],
        },
...
async function setSecret(secretName: string, secretValue: string): Promise<string> {
    try {
        ...
        const encryptedSecret = Buffer.from(
            sodium.crypto_box_seal(secretValue, Buffer.from(publicKey, "base64"))
        ).toString("base64");
...
async function listSecrets(): Promise<string> {
    try {
        ...
        const secretsList = response.data.secrets
            .map(
                (secret: { name: string; updated_at: string }) =>
                    `• ${secret.name} (updated: ${new Date(secret.updated_at).toLocaleDateString()})`
            )
            .join("\n");
...
async function deleteSecret(secretName: string): Promise<string> {
    try {
...
async function setSecretsBatch(secrets: Record<string, string>): Promise<string> {
...
async function handleToolCall(
    ...
    switch (toolName) {
        case "github_set_secret": {
            const secretName = toolInput.secret_name as string;
            const secretValue = toolInput.secret_value as string;
        ...
        case "github_set_secrets_batch": {
            const secrets = toolInput.secrets as Record<string, string>;
